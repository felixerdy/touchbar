  ...
  render() {
    return (
      <div class="native-key-bindings element-panel">
        ...
        <input type="color" class="color" id={`${this.props.name}input_color`} value={ (this.props.color ? this.props.color : "No color") } onchange={() => this.onColorChanged()}></input>
        <input type="button" class="remove btn" onClick={() => this.onRemove()} value="Remove"/>
