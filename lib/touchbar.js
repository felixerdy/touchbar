        ...
        case "color-picker":
          ...
          myTouchBarElements.push(new TouchBarColorPicker({
            ...
            change: (color) => {
              ...
              var cbp = atom.workspace.getActiveTextEditor().getCursorScreenPosition()

              // check if cursor is at beginning of color hex string
              // otherwise insert 7 spaces that will be replaced with hex string
              var nextText = atom.workspace.getActiveTextEditor().getTextInBufferRange([[cbp.row-1, cbp.column], [cbp.row-1, cbp.column+7]])
              if(!colorRegEx.test(nextText)) {
                atom.workspace.getActiveTextEditor().insertText('       ')
              }

              // insert color string into next 7 range
              atom.workspace.getActiveTextEditor().setTextInBufferRange([[cbp.row-1, cbp.column], [cbp.row-1, cbp.column+7]], color)
              // set cursor position to begin of color hex code
              atom.workspace.getActiveTextEditor().setCursorScreenPosition(cbp)
            }
          }))
          break;
